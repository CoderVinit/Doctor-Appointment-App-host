...
const Notifications = () => {
  ...
  const markAllAsSeen = async () => {
    try {
      ...
      const response = await axios.post(`${window.location.origin}/api/user/mark-all-notifications-as-seen`, { userId: user._id }, {
        headers: {
  ...
  const deleteAllNotification = async () => {
    try {
      dispatch(showLoading());
      const response = await axios.post(`${window.location.origin}/api/user/delete-all-notifications`, { userId: user._id }, {
        headers: {
          Authorization: "Bearer " + localStorage.getItem('token')
        }
      });
